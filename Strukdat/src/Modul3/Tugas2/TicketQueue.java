...
class FirstLastList {
    ...
    public void insertLast(Link.Ticket dd) {
    ...
    public Link.Ticket deleteFirst() {
        if (!isEmpty()) {
            Link.Ticket temp = first.data;
            first = first.next;
            if (first == null) {
                last = null;
            }
...
class LinkQueue {
    ...
    public void enqueue(String name, int quantity) {
        Link.Ticket newTicket = new Link.Ticket(name, quantity, theList.last != null ? theList.last.data.ticketNumber + 1 : 1);
        theList.insertLast(newTicket);
        System.out.println("Ticket number " + newTicket.ticketNumber + " added to the queue.");
    }

    public Link.Ticket dequeue() {
        ...
        do {
            ...
            switch (choice) {
                ...
                case "2":
                    ...
                    break;
                case "3":
                    Link.Ticket canceledTicket = ticketQueue.dequeue();
